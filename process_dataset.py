...
from utils_dataset import ResizeDataset, aplicar_artefatos_dataset, BrainTumorDataset, aplicar_rotacao_dataset, get_targets, shuffle_pacientes
...
def process_dataset_train_valid_test(
    ...
    if not shuffle_pacientes_flag:
        train_set = process_dataset(path / "train", img_size, funcao_geradora_artefato,
                                nivel_degradacao, nivel_aleatorio_teto, nivel_aleatorio, num_classes)
    else:
        train_set = process_dataset(path / "train", img_size, funcao_geradora_artefato,
                                nivel_degradacao, nivel_aleatorio_teto, nivel_aleatorio, num_classes, augmentation=False)
    valid_set = process_dataset(path / "valid", img_size, funcao_geradora_artefato,
                                nivel_degradacao, nivel_aleatorio_teto, nivel_aleatorio, num_classes, augmentation=False)
    test_set = process_dataset(path / "test", img_size, funcao_geradora_artefato,
                                nivel_degradacao, nivel_aleatorio_teto, nivel_aleatorio, num_classes, augmentation=False)

    if shuffle_pacientes_flag:
        train_set, valid_set, test_set = shuffle_pacientes(train_set, valid_set, test_set)
...
def generate_dataloader(dataset,
    ...
    if not balancear_dataset:
        gen = DataLoader(dataset, batch_size=batch_size, shuffle=True, pin_memory=True, num_workers=0, drop_last=True) # num_workers=0 para evitar problemas do docker # , drop_last=True
        return gen
    ...
    example_weights = [class_weights[e] for e in get_targets(dataset)] # Precisamos criar um array com um peso para cada imagem do dataset final(pode ter mais imagens que o final)
    sampler = WeightedRandomSampler(example_weights, len(dataset)) # E cria um sampler

    gen = DataLoader(dataset, batch_size=batch_size, pin_memory=True, num_workers=0, sampler=sampler, drop_last=True) # drop_last=True
    return gen
